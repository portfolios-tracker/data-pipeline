...
REQUIRED_COLUMNS = {"ticker", "trading_date", "adjusted_close", "raw_close", "adj_factor"}
...
class TestNoAdjustment:
    ...
    def test_adjusted_equals_raw_with_no_events(self):
        ...
        assert result["adjusted_close"].tolist() == pytest.approx([10000.0, 10200.0, 9800.0])
...
class TestCashDividend:
    ...
    def test_prices_before_ex_date_adjusted(self):
        ...
        result = calculate_adjusted_prices(prices, divs, "VCB").set_index("trading_date")

        assert result.loc[date(2024, 1, 3), "adjusted_close"] == pytest.approx(9500.0 * expected_factor)
        assert result.loc[date(2024, 1, 4), "adjusted_close"] == pytest.approx(9800.0 * expected_factor)
    ...
    def test_oversized_dividend_guard_skips_event(self):
        ...
        prices = make_prices([
            ...
            ("2024-01-05", 1000.0),    # ex-date; dividend=50*100=5000 > 1000 -> skipped
        ])
        result = calculate_adjusted_prices(
...
class TestStockDividend:
    ...
    def test_prices_before_ex_date_adjusted(self):
        ...
        result = calculate_adjusted_prices(prices, divs, "FPT").set_index("trading_date")

        assert result.loc[date(2024, 3, 1), "adjusted_close"] == pytest.approx(22000.0 * expected_factor, rel=1e-5)
        assert result.loc[date(2024, 3, 4), "adjusted_close"] == pytest.approx(23000.0 * expected_factor, rel=1e-5)
...
class TestMultipleEvents:
    ...
    def setup_method(self):
        ...
        self.result = (
            calculate_adjusted_prices(self.prices, self.divs, "MWG")
            .set_index("trading_date")
        )
        self.fa = 1.0 / 1.10
        self.fb = 1.0 / 1.20
    ...
    def test_after_all_events_unchanged(self):
        assert self.result.loc[date(2024, 6, 1), "adjusted_close"] == pytest.approx(15000.0)
    ...
    def test_raw_close_never_modified(self):
        originals = {
            date(2024, 1, 10): 10000.0, date(2024, 2, 10): 11000.0,
            date(2024, 3, 1): 12000.0,  date(2024, 4, 1): 13000.0,
            date(2024, 5, 1): 14000.0,  date(2024, 6, 1): 15000.0,
        }
